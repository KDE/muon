    ...
    , m_state(Installed)
{
    ...
    rx.exactMatch(bundlePath);
    m_info.name = rx.cap(1);
    m_info.version = rx.cap(2);
    m_info.release = rx.cap(3).toInt();
    m_info.completeName = rx.cap(1) + "-" + rx.cap(2) + "-" + rx.cap(3);
    m_info.size = QFileInfo(parent->repoDir().filePath(m_info.completeName) + "-" + parent->architecture() + ".cb").size();

    KDesktopFile desktopFile(parent->cacheDir().filePath(bundlePath));
